...
import { api, getHighQualityImage, formatDuration, normalizeSong, BASE_URL } from '@/lib/api';
...
export default function SongClient({ songId, initialSong }: SongClientProps) {
  const { data: songData, isLoading } = useQuery({
    queryKey: ['song', songId],
    queryFn: async () => {
      const res = await fetch(`${BASE_URL}${api.songDetails(songId)}`);
      return res.json();
    },
    enabled: !!songId,
    initialData: initialSong ? { success: true, data: [initialSong] } : undefined,
  });

  const { data: suggestionsData } = useQuery({
    queryKey: ['song-suggestions', songId],
    queryFn: async () => {
      const res = await fetch(`${BASE_URL}${api.songSuggestions(songId)}`);
      return res.json();
    },
    enabled: !!songId,
  });
  ...
  const handleLike = () => requireAuth(() => {
    ...
    toast.success(isFavorite ? 'Removed from liked songs' : 'Added to liked songs');
  });
