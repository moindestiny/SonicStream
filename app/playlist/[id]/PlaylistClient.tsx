...
import { api, getHighQualityImage, formatDuration, normalizeSong, getHighQualityDownloadUrl, BASE_URL } from '@/lib/api';
import { Play, Pause, Heart, Clock, ChevronDown, Download, Plus, Share2 } from 'lucide-react';
...
export default function PlaylistClient({ playlistId, initialPlaylist }: PlaylistClientProps) {
  ...
  const {
    ...
    queryFn: async ({ pageParam = 1 }) => {
      const res = await fetch(`${BASE_URL}${api.playlistDetails(playlistId, pageParam, PAGE_SIZE)}`);
      return res.json();
    },
    getNextPageParam: (lastPage, allPages) => {
      if (!lastPage?.data?.songs || lastPage.data.songs.length < PAGE_SIZE) return undefined;
      return allPages.length + 1;
    },
    initialPageParam: 1,
    enabled: !!playlistId,
    initialData: initialPlaylist ? {
      pages: [{ success: true, data: initialPlaylist }],
      pageParams: [1]
    } : undefined,
  });
  ...
  if (!data?.pages?.[0]?.data) return <div className="p-8 text-center" style={{ color: 'var(--text-muted)' }}>Playlist not found</div>;

  const playlist = data.pages[0].data;
  const songs = data.pages.flatMap(page => page.data.songs || []).map(normalizeSong);
      ...
      <div className="px-3 md:px-8 relative z-10">
          ...
          {songs.map((song: any, index: number) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex justify-end text-sm font-medium w-16" style={{ color: 'var(--text-muted)' }}>{formatDuration(song.duration)}</div>
              </div>
            );
          })}
        </div>
        ...
        {isFetchingNextPage && (
          <div className="space-y-1 mt-4">
            {[...Array(5)].map((_, i) => <div key={i} className="h-14 skeleton-loading rounded-xl" />)}
          </div>
        )}
        ...
        {hasNextPage && (
          <div className="flex justify-center pt-8">
            <button onClick={() => fetchNextPage()} disabled={isFetchingNextPage}
              className="flex items-center gap-2 px-6 py-2.5 rounded-full font-semibold text-sm transition-all disabled:opacity-50 glass-card"
